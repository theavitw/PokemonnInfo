...
const Home = () => {
  ...
  return !loading ? (
    <>
      <div
        className={`xl:px-24 px-10 mt-8 py-4 sticky top-0 z-[99999] dark:bg-slate-900  ${
          openPokemonInfo ? "xl:pl-10 w-[70%] relative left-[30%]" : ""
        }`}
      >
        <input
          value={search}
      ...
      <div
        className={`grid gap-4 sm:grid-cols-2 xl:px-24 px-10 py-4 dark:bg-slate-900 
                        ${
                          openPokemonInfo
                            ? "lg:grid-cols-3 xl:pl-10 w-[70%] relative left-[30%]"
                            : "lg:grid-cols-4"
                        }`}
      >
        {search
          ? pokemons?.results
              ...
              ?.filter((pokemon) => {
                if (generation) {
                  ...
                  return (
                    pokemonId > Number(offset) &&
                    pokemonId <= Number(offset) + Number(limit)
                  );
                }
                return true;
              })
              ?.filter((pokemon) => {
                if (gameVersion) {
                  clr = gameVersion;
                  const pokemonId = pokemon.url.split("/")[6];
                  return VersionData[gameVersion].Pokemon.some(
                    (kantoPokemon) => kantoPokemon.url === pokemonId
                  );
                }
                return true;
              })
      ...
      {openPokemonInfo ? (
        <PokemonInfo
          ...
          setInfoOpen={setOpenPokemonInfo}
        />
      ) : (
        <></>
      )}
